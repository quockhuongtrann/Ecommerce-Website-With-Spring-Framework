...
function updateOrderAmounts() {
	...
	$(".cost-input").each(function(e) {
		...
		productCost = getNumberValueRemovedThoudsandSeparator(costInputField);
	...
	$(".subtotal-output").each(function(e) {
		productSubtotal = getNumberValueRemovedThoudsandSeparator($(this));
	...
	$(".ship-input").each(function(e) {
		productShip = getNumberValueRemovedThoudsandSeparator($(this));
		shippingCost += productShip;
	});
	
	setAndFormatNumberForField("shippingCost", shippingCost);
	
	tax = getNumberValueRemovedThoudsandSeparator(fieldTax);
...
function updateSubtotalWhenPriceChanged(input) {
	priceValue = getNumberValueRemovedThoudsandSeparator(input);
...
function updateSubtotalWhenQuantityChanged(input) {
	...
	rowNumber = input.attr("rowNumber");
	priceValue = getNumberValueRemovedThoudsandSeparator($("#price" + rowNumber));
	newSubtotal = parseFloat(quantityValue) * priceValue;
...
function getNumberValueRemovedThoudsandSeparator(fieldRef) {
